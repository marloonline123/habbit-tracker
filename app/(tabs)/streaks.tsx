...
export default function StreaksScreen() {
  ...
  useEffect(() => {
    if (user) {
      const habitsChannel = `db.${DATABASE_ID}.collections.${HABBITS_COLLECTION_ID}.documents`;
      const habitsSubscription = client.subscribe(
        ...
        (response) => {
          if (
            ...
            response.events.includes(
              "db.*.collections.*.documents.*.delete"
            )
          ) {
            fetchHabits();
          }
      ...
      const completionsChannel = `db.${DATABASE_ID}.collections.${HABBITS_COMPLETIONS_COLLECTION_ID}.documents`;
        ...
        <View style={styles.rankingContainer}>
          ...
          {rankedHabits.slice(0, 3).map((item, key) => (
            <View key={key} style={styles.rankingRow}>
              ...
              <Text style={styles.rankingHabit}> {item.habit.name}</Text>
